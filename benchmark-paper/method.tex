...
\revision{
The code related to this study is available on Github
(\href{https://github.com/azazel7/paper-benchmark}{here}).
The repository contains all the source code we used to run the experiment. In
particular, it contains the Makefile to compile the benchmark, the Python script
we used to organize the execution and plot the results, and the
...
The repetitions were organized with plain Python code and the plots
were generated using \texttt{Seaborn}, \texttt{Matplotlib}, and \texttt{Panda}.
}
...
\revision{
Through the README, we provided the commands to download, compile, and run the
experiment. Additionally, we provided the command to plot the results.
}
...
Linux 8 with the Linux kernel 4.18.
}
